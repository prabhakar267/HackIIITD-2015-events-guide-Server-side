	...
	if($id > 0){
		...
		if($query_run = mysqli_query($connection,$query)){
			if(mysqli_num_rows($query_run) != 1){
				...
				$max_score_index++;
				$query = "
					SELECT X.description, X.id, X.name, X.start_time, X.latitude, X.longitude, X.venue, X.image_url
				...
				if($query_run = mysqli_query($connection, $query)){
					while($query_row = mysqli_fetch_assoc($query_run)){
						...
						$response = array(
							...
							'venue'			=> trim($query_row['venue']),
							'image'			=> trim($query_row['image_url'])
						);
		...
		$query = "SELECT `id`,`name`,`start_time`,`latitude`,`longitude`,`description`,`venue`,`image_url` FROM `events` WHERE `start_time`>='$current_time' ORDER BY `start_time` ASC LIMIT 20";
		if($query_run = mysqli_query($connection,$query)){
			while($query_row = mysqli_fetch_assoc($query_run)){
				$short_description = htmlspecialchars_decode(trim(substr($query_row['description'], 0, 300)));
				$response = array(
